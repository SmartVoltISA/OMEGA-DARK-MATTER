...
Reconstructs the frozen R3 model, performs frozen single-edge necessity tests,
and runs training-only backward elimination under preregistered tau=0.01.
"""
import json, math, random, platform, sys
import numpy as np
from scipy.optimize import nnls
SEED=20260911; N=24; TRAIN=40; TEST=20; LENGTH=80; K=.05; GAMMA=.02; SIGMA=.005; EDGE_P=.12; TAU=.01
LAMBDAS=(.1,.3,1,3,10,30,100); EDGES=np.array([(i,j) for i in range(N) for j in range(i+1,N)],int); P=len(EDGES)
B=np.zeros((N,P))
...
def trajectory(W,seed):
 ...
 return out
def stats(trs):
 D=np.vstack([tr[:-2,EDGES[:,0]]-tr[:-2,EDGES[:,1]] for tr in trs]); Y=np.vstack([(tr[2:]-2*tr[1:-1]+tr[:-2])+GAMMA*(tr[1:-1]-tr[:-2]) for tr in trs]); G=(K*K)*(B.T@B)*(D.T@D); b=K*np.sum(D*(Y@B),axis=0); return G,b,float(np.sum(Y*Y)),len(Y)*N
def fit(G,b,yty,lam):
 vals,U=np.linalg.eigh(G); keep=vals>1e-10; A=np.sqrt(vals[keep])[:,None]*U[:,keep].T; q=(U[:,keep].T@(b-lam))/np.sqrt(vals[keep]); w,_=nnls(A,q,maxiter=100000); return w,float(yty-2*w@b+w@G@w)
def fit_r3(trs):
...
def rollout_rmse(W,trs,h):
 ...
 for tr in trs:
  for t in range(1,LENGTH-h):
   prev,cur=tr[t-1],tr[t]
   for _ in range(h):
    v=cur-prev; nxt=cur+v-K*(W.sum(1)*cur-W@cur)-GAMMA*v; prev,cur=cur,nxt
...
def deletion_table(W,test,active_edges):
 ...
 for idx,(a,b) in enumerate(active_edges):
  wd=W.copy(); wd[a,b]=wd[b,a]=0; vals=[rollout_rmse(wd,test,h) for h in (1,2,5,10)]; d=[vals[i]-base[i] for i in range(4)]
  rows.append({'edge_id':idx,'a':int(a),'b':int(b),'delta_h1':d[0],'delta_h2':d[1],'delta_h5':d[2],'delta_h10':d[3],'provisionally_necessary':bool(d[0]>TAU)})
 return base,rows
...
if __name__=='__main__':
 trueW=graph(SEED); train=[trajectory(trueW,SEED+1000+i) for i in range(TRAIN)]; test=[trajectory(trueW,SEED+2000+i) for i in range(TEST)]; W,best,grid=fit_r3(train); active=EDGES[np.flatnonzero(W[np.triu_indices(N,1)]>1e-8)]; base,table=deletion_table(W,test,active); cores={}
 ...
 print(json.dumps({'status':'EXECUTED','config':{'seed':SEED,'N':N,'train':TRAIN,'test':TEST,'length':LENGTH,'K':K,'gamma':GAMMA,'sigma':SIGMA,'edge_p':EDGE_P,'tau':TAU,'lambda_grid':LAMBDAS},'runtime':{'python':sys.version,'numpy':np.__version__,'scipy':__import__('scipy').__version__,'platform':platform.platform()},'r3_selected':{'lambda':best['lambda'],'active_edges':best['active_edges']},'frozen_full_test_rmse':base,'frozen_deletion':table,'cores':cores},indent=2))
